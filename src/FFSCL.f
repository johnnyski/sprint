     ...
     &     IDPTR,IFC,MTFIEL,IBAD,IOVER,ITIFLG,N,I,
...
C     
      PARAMETER (NID=256,MAXFLD=8,MXCNT=500)
      DIMENSION IBUF(8500),THVAL(2,1024),KOUT(8500),ID(NID)
...
C     
            ...
            IF (ITYP.EQ.1) THEN
               ...
               IF (CTDBMXV(1).EQ.-32768) THEN
                  WRITE(*,720)
 720              FORMAT(/,5X,'+++NEED CALIBRATION DATA FOR DBMXV ',
     X                 'FIELD+++')
                  STOP
               END IF
            ELSE IF (ITYP.EQ.3) THEN
