...
C
 ...
 45            FORMAT(//,5X,'+++UNKNOWN FILTER TYPE:',A8,' +++'/)
            ...
            ELSE
               IPROC(INDXFLT)='NONE'
            END IF
            IF (KRD(5).EQ.'FILL') NSCTP(INDXFLT)='FILL'
         ELSE
 ...
 50            FORMAT(/,5X,'+++ONLY ',I3,' FIELDS CAN BE FILTERED+++')
            ...
            ELSE
               IPROC(NUMFILT)='NONE'
            END IF
            IF (KRD(5).EQ.'FILL') NSCTP(NUMFILT)='FILL'
         END IF
...
C
 ...
 133  FORMAT(/,5X,I2,' FIELDS TO BE FILTERED...')
      DO I=1,NUMFILT
         WRITE(*,125)IFLDS(I),NFILT(IFLTYP(I)),IPROC(I)
 125     FORMAT(/20X,'          FIELD NAME: ',A8,
     X          /20X,'         FILTER TYPE: ',A8,
     X          /20X,'  SPECIAL PROCESSING: ',A8)
         IF (NSCTP(I).EQ.'FILL') THEN
            WRITE(*,127)
 127        FORMAT(/20X,'FILLING OF MISSING DATA POINTS TURNED ON',/)
         ELSE
            WRITE(*,*)
         END IF
      END DO
