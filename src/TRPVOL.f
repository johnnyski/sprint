      SUBROUTINE TRPVOL(ICART,ICTAB,NPREC,ICOB,NDX,NDY,NDZ,ZRTAB,
     X     MXCRT,IBLV,NRCBF,NST,DASANG,IFLAT,NUMFILT,XRTAB,IVREF)
C
C        PREPARES FOR ACTUAL INTERPOLATION TASK
C
...
C
     ...
     X     IVREF(IVDIM,2),IDATCB(MAXFLD),ICTAB(4),KZBUF(2,2),
     X     IBKNT(4,2),IVV2(IDIM),XRTAB(MXCRT,2)
...
C     
      ...
      ELSE IF (ICOPLANE.EQ.4) THEN
         IF (IRHICRS.EQ.0 .OR. IRHICRS.EQ.3) THEN
            TANLO=TAN(ELB(KPEL)*ATR)
            TANHI=TAN(ELB(KPEL+ISD)*ATR)
            IF (ELB(KPEL).GT.90.0 .AND. ELB(KPEL).LT.270.0) THEN
               TANTMP=-TANLO
               TANLO=-TANHI
               TANHI=TANTMP
            END IF
         ELSE IF (IRHICRS.EQ.1 .OR. IRHICRS.EQ.2) THEN
            IF (ELB(KPEL+ISD).LT.225.0) THEN
C
...
C     360 SCANS ONLY
      IF(IS360)ICSAV=IC
 20   CONTINUE
      IF(IBKNT(2,KL).EQ.0) GO TO 30
C     
C     INITIALIZATION FOR LOCATIONS BELOW THIS PPI SCAN
C     
      KINT=KINT+1
     ...
     X        NST,IBELOW)
      ELSE
         JLV=2.+(IDSV*0.5)
         CALL SCNSET(KZBUF(1,KL),IBKNT(1,KL),IBLV(1,1,1,KL),NRCBF,
     X        KZV(KZP),IDSV,JLV,ILO,LPMN,LPMX,KZLO,IS360,IZAD,ICROSS,
     X        NST,IBELOW)
      END IF
      IF(NST.NE.0 .AND. ICOPLANE.NE.4) GO TO 800
C     360 SCANS ONLY
...
C     
      DO 37 I=IZ1,IZ2
         ...
         ELSE IF (ICOPLANE.EQ.4) THEN
            ZADJ=ZRTAB(I,1)
         END IF
         IF((ZADJ.GT.RNG.OR.ZADJ.LT.0.0) .AND. 
...
C         IVREF(IHI,KH)=MN
     ...
     X           IBLV(1,4,IHI,KH),IVV2)
            CALL IPKDAT(ICOB(1,1,IX,IY,I),IDATCB,NOF)
         END IF
         IHI=IHI+IDS
...
C         IVREF(IHI,KH)=MN
 ...
 55   CONTINUE
C     
C     INTERPOLATE LOCATIONS BELOW THE CURRENT PPI SCAN AND
C     WRITE THEM TO OUTPUT BUFFERS.
C     
      IF (IBELOW.EQ.0) THEN
C
