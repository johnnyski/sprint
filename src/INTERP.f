...
C               VARIANCE, CORRELATION, AND TIME FIELD CARDS
C               --------- ------------ --- ---- ----- -----
C
C   FLDNAM     NAME OF FIELD                 P2     4 CHARACTERS LONG
C
...
C
C
C   TEST FOR MORE THAN 4 FIELD CARDS:
C
      I = I+1
      IF (I.GT.MAXFLD) THEN
         PRINT 113
...
C     FOR THE DIFFERENT FIELDS. NEED TO INVOKE THE FUNCTION ITPFLD.
C
...
C
      IF (KFLDTP.EQ.0) THEN
         PRINT 224, IFIELD(I)
224      FORMAT(/1X,'+++  WARNING  +++'/
     X           1X,'UNABLE TO DETERMINE FIELD TYPE FOR FIELD = ',A8)
...
C     
 ...
 715           FORMAT(/,23X,'THIS FIELD WILL BE SET TO BAD IF ',A8,
     X                ' LESS THAN ',F8.2,' OR GREATER THAN ',F8.2)
            ...
            ELSE 
               PRINT 720
 720           FORMAT(/5X,'+++ ONLY ALLOWED 2 THRESHOLD FIELDS +++')
               STOP 
            END IF
         END IF
...
C
         ...
         END IF
         PRINT 232, IACTIV
232     FORMAT(23X,'DISCARDING OF ALL NCAR-FLAGGED BAD VELOCITIES: ',A8)
C
         IACTIV = ISWICH(2)
