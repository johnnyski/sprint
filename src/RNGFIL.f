...
C   ELTOL      ELEVATION TOLERANCE           P2
C   MNBEM      MINIMUM NUMBER OF BEAMS       P3
C   RUSR1      STARTING RANGE                P4     IN KM
C   RUSR2      ENDING RANGE                  P5     IN KM
C   RNOTUS     BEGINNING GATE                P6     CORRECTION IN M
C   DRGUS      GATE SPACING                  P7     IN M
...
888   FORMAT(/8X,'                  STARTING RANGE: ',A14
     ...
     X       /8X,'             ELEVATION TOLERANCE: ',F7.2,' DEGREES',
     X       /8X,'MINIMUM NUMBER OF BEAMS PER SCAN: ',I5
     X       /8X,'CORRECTION TO FIRST GATE ON TAPE: ',A14
     X       /8X,'            GATE SPACING ON TAPE: ',A14
     X       /8X,'          RADAR CONSTANT ON TAPE: ',A14
     X       /8X,'        NYQUIST VELOCITY ON TAPE: ',A14)
C
C
C        ESTABLISH FIXED ELEVATION ANGLE MODE
C
 ...
 500  FORMAT(A8)
  ...
  117 FORMAT(A2,1X,F5.0)
      ...
      IF(IELT.EQ.'MD') THEN
         PRINT 889, I
  889    FORMAT(8X,'FIXED ELEVATION ANGLE OF EACH SCAN WILL BE ',
     X          'RECOMPUTED USING THE MOST FREQUENTLY'/8X,
     X          '   OCCURRING ELEVATION OF ALL THE BEAMS IN THE SCAN',
     X      /8X,'   EXCLUDING THE FIRST AND LAST',I5,'.'//)
      ELSE IF(IELT.EQ.'MN') THEN
         PRINT 890, I
  890    FORMAT(8X,'FIXED ELEVATION ANGLE OF EACH SCAN WILL BE ',
     X          'RECOMPUTED USING THE'/8X,'   MEAN ELEVATION OF',
     X          ' ALL BEAMS IN THE SCAN'/8X,'   EXCLUDING THE',
     X          ' FIRST AND LAST',I5,'.'//)
      ELSE
         IRCFXL=IBL
         PRINT 891
  891   FORMAT(8X,'TAPE VALUE OF FIXED ELEVATION ANGLE WILL BE USED.'//)
      END IF
