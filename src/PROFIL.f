...
887   FORMAT(5X,'------- -- ------- ------- ')
     ...
     X     TABOUT
888   FORMAT(/5X,'                      DATE (YYMMDD): ',I6
     X       /5X,'            BEGINNING TIME (HHMMSS): ',I6
     X       /5X,'               ENDING TIME (HHMMSS): ',I6
     X       /5X,'SPECIAL VOLUME PROCESSING PERFORMED: ',A8
     X       /3X,'ELEVATION SCANS WILL BE DETERMINED BY: ',A12,
     X       /3X,' DISCARD BEAMS FLAGGED AS TRANSITIONS: ',A46,
     X       /5X,'          USER SUPPLIED SWEEP TABLE: ',A8)
      IF(ISWMTH.EQ.1) THEN
         PRINT 889, FXSTOL
  889    FORMAT(3X,'FIXED ANGLE ELEVATION TOLERANCE (DEG): ',F8.2)
      END IF
C
      IF (KDAY.LE.0) CALL TPQERX(326,1)
      RETURN
 105  FORMAT (5X,'***ERROR IN PRO COMMAND  ',A10,'=',I10)
      END
